...
La skill tiene una única fuente en `.agents/skills/fast-forward-mode/source/` y dos distribuciones deterministas. Codex usa `.agents/skills/fast-forward-mode/SKILL.md`; Hermes usa `.agents/skills/fast-forward-mode/distributions/hermes/fast-forward-mode/SKILL.md`, que se instala como `SKILL.md` dentro del `skills/fast-forward-mode/` físico del perfil activo bajo `~/.hermes/profiles/<perfil>/`. Hay que usar el perfil que efectivamente ejecuta CompAII, no adivinar su nombre. `scripts/render_distributions.py --check` comprueba que ambas salidas coincidan con la fuente. Reiniciar o recargar la sesión para volver a descubrir skills y verificar `$fast-forward-mode` en Codex o `/fast-forward-mode` en Hermes con una invocación sin mutaciones. El paquete contiene instrucciones, metadata y un renderer local sin privilegios: no concede permisos ni ejecuta operaciones del proyecto.
