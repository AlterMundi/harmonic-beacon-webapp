...
Las skills portables tienen una fuente por skill bajo `.agents/skills/<skill>/source/` y dos distribuciones deterministas. Codex usa `.agents/skills/<skill>/SKILL.md`; Hermes usa `.agents/skills/<skill>/distributions/hermes/<skill>/SKILL.md`, que se instala como `SKILL.md` dentro del `skills/<skill>/` físico del perfil activo bajo `~/.hermes/profiles/<perfil>/`. Hay que usar el perfil que efectivamente ejecuta CompAII, no adivinar su nombre. `.agents/skills/scripts/render_distributions.py --check` comprueba todas las salidas contra sus fuentes. Reiniciar o recargar la sesión para volver a descubrir skills y verificar `$<skill>` en Codex o `/<skill>` en Hermes con una invocación sin mutaciones. El paquete contiene instrucciones, metadata y un renderer local sin privilegios: no concede permisos ni ejecuta operaciones del proyecto.
