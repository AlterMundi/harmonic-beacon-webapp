...
Harmonic Beacon live weekend — Saturday 2026-08-01 (EN) and Sunday 2026-08-02 (ES).
This is the operator's playbook: who owns each failure, how it is detected, the
first action, when to fall back or abort, what attendees are told, and who
decides refunds. It assumes the architecture in
...
|---|---|---|
| **Incident Commander (IC)** | Admin | Declares incidents, fallback, and abort. Sole refund decision owner. Runs all host-level commands. |
...
- The IC is the only person who may declare the fallback meeting or an abort,
  and the only person who decides refunds.
...
- Fallback meeting link confirmed live (§5.12) — click it, do not just read it.
...
- Never improvise URLs. The only links attendees ever receive are the event
  URL and the pre-created fallback meeting URL.
...
- **Refund decision owner:** IC — only if the attendee is never admitted and the event ran. Individual case, post-event.
...
- **Refund decision owner:** IC — a wrongly revoked paying attendee gets reinstated first, refunded only if reinstatement fails.
...
- **Refund decision owner:** IC — no refund for a bed gap under 15 minutes; longer gaps are assessed post-event with the attendance ledger.
...
- **Fallback/abort threshold:** 10 minutes without Julián → IC announces a short intermission (message below). 20 minutes → IC invokes the fallback meeting (§5.12) so the session continues with voice, or aborts per §6 if the fallback is also impossible.
- **Attendee message:**
...
- **Refund decision owner:** IC — session resumed within 20 minutes: no refund. Fallback meeting delivered: no refund (disclosed fallback policy). Otherwise partial/full per §7.
...
- **Refund decision owner:** IC — an abuser is never refunded; a wrongly removed attendee is reinstated and, if the session was missed, refunded.
...
- **Refund decision owner:** IC — no refund if the session completed in audio-only; refunds only if paying attendees were turned away (§7).
...
- **Fallback/abort threshold:** App down > 15 minutes during a session → IC invokes the fallback meeting (§5.12). Attendees already in the LiveKit room keep media until their token needs renewal; tell them not to refresh.
- **Attendee message:**
...
- **Refund decision owner:** IC — per §7 thresholds.
...
- **Fallback/abort threshold:** DB down > 15 minutes during a session → fallback meeting (§5.12). Data corruption → restore from the T-2h backup before deciding anything else; that decision is the IC's.
- **Attendee message:**
...
- **Refund decision owner:** IC — per §7.
...
- **Fallback/abort threshold:** LiveKit down > 10 minutes during a session → IC invokes the fallback meeting (§5.12). A TURN-only failure (restrictive networks can't connect) is not an abort: affected attendees are support cases (§5.1 channel), pre-verified at T-2h.
- **Attendee message:**
...
- **Refund decision owner:** IC — per §7; vendor downtime is our risk, not the attendee's.
...
- **Refund decision owner:** IC — never refundable.

### 5.12 Total platform failure → fallback meeting
...
- **First action:** IC posts the **pre-created fallback meeting link** (created in WS6-01, verified at T-2h) to the attendee contact channel and to the event page if it still serves. Never improvise a new URL during the incident.
- **Fallback/abort threshold:** The fallback meeting *is* the fallback. If Julián cannot join it either within 10 minutes, the IC aborts (§6).
- **Attendee message:**
...
- **Refund decision owner:** IC — a delivered fallback session is not refunded (disclosed fallback policy). A fallback that never happened is an abort (§6).
...
- Julián cannot reach attendees by any path (platform + fallback meeting) for
...
1. IC declares the abort on the staff channel; timestamp it.
...
- **Sole decision owner: the Incident Commander.** Julián is consulted on
  ...
  attendance/incident ledger. Nobody else promises refunds to attendees —
...
- **Standard outcomes:**
  - Session delivered (incl. fallback meeting, incl. short bed/tapestry gaps): no refund.
  ...
  - Abuser removed under §5.5: no refund.
...
[time ART] refund decision (if any) — decided by IC
```
