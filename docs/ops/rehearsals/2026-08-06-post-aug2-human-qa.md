...
| Gap between main and deployed | Exactly 1 commit: `341c15c` (SessionGuidance, PR #206) |
...
Verified facts at preparation time (2026-08-06, `git merge-base --is-ancestor`
against the deployed SHA, plus `/api/health` and `/api/health/ready`):

- The deployed SHA **does contain** PR #187 (`388fe8f`, chat UI) and PR #192
  (`d547e96`, E2E fixes). Contributions are live in production.
- The deployed SHA **does not contain** PR #206 (`341c15c`, SessionGuidance).

**Fail-closed rule:** if `/api/health` at execution time does not show a
candidate containing PRs #187, #192 **and #206**, mark every row
**BLOCKED** and attribute no result to the new design. Execution is equally
blocked until real humans fill every role in §2 — a prepared sheet is not an
executed rehearsal.
