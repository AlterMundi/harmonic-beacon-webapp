...
- Session chat ships as session contributions (questions and emotions, CHAT-01): plain text, per-message named/anonymous visibility, and a per-participant sliding-window anti-flood limit (5 submissions per 60 seconds per session) that never keys on IP — see [SESSION_CONTRIBUTIONS.md](./architecture/SESSION_CONTRIBUTIONS.md). The report-this-message UI does not exist yet; it lands with moderation in CHAT-02. **[Planned — CHAT-02]** *(report UI only)*
