...
- Content generated by AI must be disclosed. The labelled field it will be disclosed in does not exist — upload collects a title, a description, tags and a mix position, and nothing else — so there is nowhere to record the disclosure at upload today. **[Planned — unscheduled]**
...
> **Unresolved:** which flag combination "hidden" is. This workflow has specified
> `isHidden=true` with `isPublished=false`; the lifecycle table in
> [BUSINESS_RULES.md §2.1](../BUSINESS_RULES.md) keeps `isPublished: true` for a
> Hidden meditation. Those are different invariants, and a visibility filter
> written from one behaves differently under the other. Neither has been chosen.
> The code meanwhile does its own thing: the Admin hide toggle writes `isHidden`
> alone and leaves `isPublished` untouched, which for already-published content
> lands on the table's combination rather than this document's. Do not write a
> query against any of the three until the invariant is settled.
