...
const fixture = () => ({schemaVersion:'harmonic-beacon.delivery-authorization.v2',
 ...
 transitionAuthorizationSha256:null, impactStateSha256:digest, impactPlanSha256:digest, verbs:['prepare','preflight','status'],
...
test('closed delivery schema rejects mismatched binding, time, target, operation and verbs', () => {
 ...
 for(const key of ['sourceSha','sourceTree','candidateManifestSha256','baseManifestSha256','candidateRunId','candidateRunAttempt','deliveryRunId','deliveryRunAttempt','workflowPath','workflowRef','environment','target','operation','configSha256','transitionAuthorizationSha256','impactStateSha256','impactPlanSha256','laneState']) {
...
test('production promote requires a transition digest; rollback uses only rollback verb',()=>{
 ...
 a.verbs=['rollback'];a.transitionAuthorizationSha256=null;a.impactStateSha256=null;a.impactPlanSha256=null;trusted.validateDeliveryAuthorization(bytes(a),{}, {now});
});
...
test('fresh rollback permits only its exact durable intent; expired intent resumes and replay has no new authority',()=>{
 ...
 const rollback={...original,operation:'rollback',deliveryRunId:'99',verbs:['rollback'],transitionAuthorizationSha256:null,impactStateSha256:null,impactPlanSha256:null};
...
test('installed rollback admission binds exact current publication and rejects expired, reused or obsolete intent',()=>{
 ...
 try {
  const state=stateFixture('b');const a={...fixture(),target:'production',environment:'production',laneState:'oci-production',operation:'rollback',verbs:['rollback'],transitionAuthorizationSha256:null,impactStateSha256:null,impactPlanSha256:null,deliveryRunId:'99',candidateManifestSha256:state.manifestSha256,authorizedAt:new Date().toISOString(),expiresAt:new Date(Date.now()+600000).toISOString()};
...
test('hosted rehearsal is protected and job dependency truth table preserves shadow and rollback', () => {
 ...
 assert.deepEqual(jobs.authorize.needs,['resolve','rehearse','impact_plan']);
 ...
 for(const [target,operation] of [['production','promote'],['shadow','promote'],['production','rollback']]) {
  ...
  assert.equal(evaluate(jobs.impact_plan.if,inputs,{},vars),operation==='promote');
  for(const result of ['success','skipped','failure','cancelled']) {
   const needs={resolve:{result:'success'},rehearse:{result:required?'success':'skipped'},impact_plan:{result}};
   assert.equal(evaluate(jobs.authorize.if,inputs,needs,vars),operation==='promote'?result==='success':result==='skipped');
  }
  for(const result of ['success','skipped','failure','cancelled']) {
