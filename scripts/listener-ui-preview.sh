...
PREVIEW_DROPIN_ES_PATH="${LISTENER_UI_PREVIEW_DROPIN_ES_PATH:-}"
...
for switch in "$PREVIEW_FREE_FOR_ALL" "$PREVIEW_REACTIVE_FIELD_LAB" "$PREVIEW_PAYPAL_CHECKOUT" \
    "$PREVIEW_MERCADO_PAGO_CHECKOUT" "$PREVIEW_LIVE_WORKBENCH" "$PREVIEW_APPLE"; do
    case "$switch" in 0|1) ;; *) echo "Preview switches must be 0 or 1." >&2; exit 2 ;; esac
done

if [ "$PREVIEW_APPLE" = 1 ] && [ "$PREVIEW_FREE_FOR_ALL" != 0 ]; then
    echo "Apple sign-in acceptance requires Free For All to be disabled." >&2
    exit 2
fi

for intro_override in \
    "Spanish:$PREVIEW_DROPIN_ES_PATH" \
    "English:$PREVIEW_DROPIN_EN_PATH"; do
    intro_language="${intro_override%%:*}"
    intro_path="${intro_override#*:}"
    if [ -n "$intro_path" ] &&
       [[ ! "$intro_path" =~ ^/media/artifacts/drop-ins/[A-Za-z0-9][A-Za-z0-9._-]{0,127}\.m4a$ ]]; then
        echo "$intro_language intro preview must name one bounded immutable .m4a artifact." >&2
        exit 2
    fi
done
...
if [ -n "$PREVIEW_DROPIN_ES_PATH" ]; then
    set_env_file_value EARLY_BIRDS_DROPIN_ES_PATH "$PREVIEW_DROPIN_ES_PATH"
fi
...
set_env_file_value BEACON_LISTENER_APPLE_ENABLED "$PREVIEW_APPLE"
