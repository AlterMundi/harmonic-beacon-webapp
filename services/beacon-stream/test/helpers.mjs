...
export function variableMetadata(overrides = {}) {
  return validateArtifact({
    schemaVersion: 2,
    artifactId: 'approved-v2',
    approval: { status: 'APPROVED', approvedAt: '2026-08-06T00:00:00.000Z', reviewRecord: 'audio-review-reference' },
    source: { masterSha256: SHA },
    derivative: { sha256: 'b'.repeat(64) },
    timing: { epochUtc: '2026-08-06T00:00:00.000Z', targetSegmentDurationSeconds: 6, segmentCount: 3, loopDurationSeconds: 16 },
    initialization: { file: 'init.mp4', bytes: 4, sha256: crypto.createHash('sha256').update('init').digest('hex') },
    segments: [
      { file: '00000.m4s', durationSeconds: 6, bytes: 3, sha256: crypto.createHash('sha256').update('one').digest('hex') },
      { file: '00001.m4s', durationSeconds: 6, bytes: 3, sha256: crypto.createHash('sha256').update('two').digest('hex') },
      { file: '00002.m4s', durationSeconds: 4, bytes: 5, sha256: crypto.createHash('sha256').update('three').digest('hex') },
    ],
    ...overrides,
  });
}
...
export async function temporaryVariableArtifact() {
  const mediaRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'beacon-stream-v2-'));
  const artifactRoot = path.join(mediaRoot, 'approved-v2');
  await fs.mkdir(path.join(artifactRoot, 'segments'), { recursive: true });
  await Promise.all([
    fs.writeFile(path.join(artifactRoot, 'segments', 'init.mp4'), 'init'),
    fs.writeFile(path.join(artifactRoot, 'segments', '00000.m4s'), 'one'),
    fs.writeFile(path.join(artifactRoot, 'segments', '00001.m4s'), 'two'),
    fs.writeFile(path.join(artifactRoot, 'segments', '00002.m4s'), 'three'),
  ]);
  await fs.writeFile(path.join(artifactRoot, 'artifact.json'), `${JSON.stringify(variableMetadata(), null, 2)}\n`);
  return { mediaRoot, artifactRoot };
}
