...
7. Validate at the appropriate boundary with evidence proportional to the
   ...
   change.
8. If a gate fails, reproduce it and identify the root cause. For a behavioral
   regression, add or strengthen the focused regression test before repairing
   it. Rerun every check invalidated by the repair, then continue. A failure
