...
export async function POST(request: Request): Promise<Response> {
    ...
    const body = await request.json().catch(() => null) as { displayName?: unknown; realName?: unknown; revision?: unknown } | null;
    const displayName = normalizeBeaconDisplayName(body?.displayName);
    const realName = normalizeBeaconRealName(body?.realName);
    if (!displayName || !realName || !Number.isSafeInteger(body?.revision) || Number(body?.revision) < 1) {
    ...
    const updated = await prisma.beaconProfile.updateMany({
        ...
        data: { displayName, realName, revision: { increment: 1 } },
    });
    ...
    const profile = await prisma.beaconProfile.findUniqueOrThrow({
        ...
        select: { accountId: true, displayName: true, realName: true, revision: true },
    });
