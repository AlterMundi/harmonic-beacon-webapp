...
import { ACCOUNT_SESSION_COOKIE, activeAccountStaticClients, isAccountHost } from '@/lib/account/config';
import { accountOrigin, accountStaticClientSecrets } from '@/lib/account/config';
...
export async function POST(request: Request): Promise<Response> {
    ...
    if (!session) return Response.json({ frontchannel: [] }, { status: 401 });
    ...
    if (!accountLogoutInitiationValid({
        token: initiation, sid: session.session.id, mode: 'current',
    ...
    await prisma.$transaction((transaction) => revokeAccountSession(transaction, session.session.id));
