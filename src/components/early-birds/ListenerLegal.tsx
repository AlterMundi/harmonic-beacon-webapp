...
export default function ListenerLegal({ withdrawalAvailable = false }: { withdrawalAvailable?: boolean }) {
        ...
        <main className="listener-shell listener-legal-shell">
            <article className="listener-legal">
                ...
                {withdrawalAvailable ? <section>
                    ...
                    <p>
                        {locale === 'es'
                            ? 'También puedes solicitar la baja del servicio sin ingresar a tu cuenta.'
                            : 'You may also request service cancellation without signing in to your account.'}{' '}
                        <ConsumerWithdrawalLink inline kind="SERVICE_CANCELLATION" />
                    </p>
                </section> : null}
            </article>
        </main>
